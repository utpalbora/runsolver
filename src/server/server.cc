/*
 https://lwn.net/Articles/230975/ saved as ELC: How much memory are applications
really using? [LWN.net].html
http://eqware.net/Articles/CapturingProcessMemoryUsageUnderLinux/index.html
saved as Capturing Process Memory Usage Under Linux.html
https://stackoverflow.com/questions/561245/virtual-memory-usage-from-java-under-linux-too-much-memory-used/561450#561450
saved as Virtual Memory Usage from Java under Linux, too much memory used -
Stack Overflow.html https://www.selenic.com/smem/ pmap
 */
...
#warning must add support for hugepages
    ...
    for (pid_t pid : pidList) {
      ...
      while (true) {
        ...
        cout << (end - beg) / pagesize << "\t" << hex << beg << "-" << end
             << dec << "  " << perm << " path=" << path << endl;
...
#warning EOF sur les derniers accès
            ...
            if (result != sizeof(desc))
              cout << "error reading page description (res=" << result
                   << "): " << strerror(errno) << "\n";
            ...
            cout << hex << p << " -> " << (present ? "Present " : "")
                 << (swap ? "Swp " : "") << (map ? "Map " : "")
                 << (excl ? "Excl " : "") << (softDirty ? "SoftD " : "") << " "
                 << ((desc >> 57) & 0xF) << " " << (desc & ((1UL << 55) - 1))
                 << dec << endl;
        ...
        cout << "=> total=" << (end - beg) / 1024 << "KB"
             << " unmapped="
             << (unmappedPages - lastUnmappedPages) * (pagesize / 1024) << "KB"
             << " ram=" << (ramPages - lastRamPages) * (pagesize / 1024) << "KB"
             << " swap=" << (swapPages - lastSwapPages) * (pagesize / 1024)
             << "KB"
             << " shared="
             << (sharedPages - lastSharedPages) * (pagesize / 1024) << "KB"
             << endl;
    ...
    cout << "unmapped=" << unmappedPages * (pagesize / 1024) << "KB\n"
         ...
         << "VSS=all="
         << (ramPages + unmappedPages + swapPages + sharedPages) *
                (pagesize / 1024)
         << "KB\n"
         << "RSS=ram+shared=" << (ramPages + sharedPages) * (pagesize / 1024)
         << "KB\n";
...
int main() {
  ...
  /*
    ...
    status ne semble pas donner la bonne valeur de RSS (incohérent avec la somme
    des RSS de smaps)
