...
public:
   ...
   */
  void addEOF(bool add) { addEOFLine = add; }
   ...
   */
  void resetTimeStamp() { gettimeofday(&tvStart, NULL); }
    ...
    while (watched.size()) {
      ...
      if (result < 0) {
        ...
        if (++watchDog < 10)
          ...
          cout << "Error in TimeStamper::timeStampLines(), select() keeps "
                  "returning errors, exiting."
               << endl;
          break;
        }
      }
    ...
    if (withCPUtime) {
      ...
      if (name)
        ...
        tstampsize = snprintf(tstampbuffer, sizeof(tstampbuffer),
                              ...
                              "%.2f/%ld.%02ld\t",
#endif
                              cputimeCopy, tv.tv_sec, tv.tv_usec / 10000);
    } else {
      // no CPU time
      if (name)
        tstampsize = snprintf(tstampbuffer, sizeof(tstampbuffer),
...
private:
  ...
  struct info {
    ...
    int outputdescr; // file descriptor to use to output the timestamped stream
                     // (when filteredOutput is false)
    AbstractFilter
        *outputfilter; // filter to write to (when filteredOutput is true)
