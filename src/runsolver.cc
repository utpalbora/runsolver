 ...
 *
 * - arrange so that display occurs each n*period seconds (and not
 * n*(period+epsilon))
...
private:
  ...
  public:
     ...
     */
    Limit(int resource) : resource(resource) { scale = 0; }
...
public:
  ...
  public:
    HardVSIZELimit(rlim_t size) : Limit(RLIMIT_AS) {
      name = "VSIZE limit (hard limit, stack expansion will fail with SIGSEGV, "
             "brk() and mmap() will return ENOMEM)";
      unit = "KiB";
      scale = 10;
      setLimit(size);
    }
...
private:
  ...
  unsigned long long int limitedOutputActivateSize = 0,
  ...
  streambuf *coutSaveBuf =
      nullptr; // backup of the cout buf (in case of a redirection)
...
public:
   ...
   */
  void setUsePTY(bool usePTY) { this->usePTY = usePTY; }
   ...
   */
  void setVarOutputFile(char *filename) { watcher.setVarOutputFile(filename); }
   ...
   */
  void setTimeStamping(bool val) { timeStamping = val; }
  ...
  void setSolverOutputLimits(unsigned long long int activateSize,
    ...
    if (!timeStamping)
      throw runtime_error("limit on the output size can only be enforced when "
                          "timestamping is on");
   ...
   */
  void setDelayBeforeKill(int seconds) { watcher.setDelayBeforeKill(seconds); }
   ...
   */
  void addLimit(Limit *limit) { limits.push_back(limit); }
  ...
  void run(char **cmd) {
...

    {
      ...
      if (res != 0) {
        ...
        cout << "This version of runsolver requires a kernel version >= 3.4 in "
                "order to use prctl(PR_SET_CHILD_SUBREAPER,...). Aborting.\n";
        exit(1);
      } else
    ...
    */
    ...
    if (timeStamping) {
      ...
      if (outputRedirectionFilename) {
        fd =
            open(outputRedirectionFilename, O_WRONLY | O_CREAT | O_TRUNC, 0644);
        if (fd < 0)
          throw runtime_error(
...
void numaInfo() {
  ...
  for (int i = 0; i < nbNodes; ++i) {
    ...
    mem /= 1024 * 1024;
    memFree /= 1024 * 1024;
    cout << "  memory of node " << i << ": " << mem << " MiB (" << memFree
         << " MiB free)\n";
  }
...
void usage(char *prgname) {
  cout << "Usage: " << prgname << endl
       ...
       << "       [-M vsize-limit | --mem-limit vsize-limit]  (deprecated, use "
          "-V or --vsize-limit)\n"
  ...
  cout << "-v filename or --var filename\n"
       << "  save the most relevant information (times,...) in an easy to "
          "parse VAR=VALUE file"
       << endl;
  cout
      ...
      << "--timestamp\n"
      << "  instructs to timestamp each line of the solver standard output "
         "and\n"
      << "  error files (which are then redirected to stdout)\n"
      << "--add-eof\n"
      << "  when timestamps are used, request to add an 'EOF' line at the end "
      ...
      << "  allocate a subset of the cores to the solver. The list contains\n"
      << "  core numbers separated by commas, or ranges first-last. This list\n"
      << "  contains the index of the selected cores in a list of core "
         "identifiers\n"
      << "  sorted by the processor they belong to. For example, if processor "
         "0\n"
      << "  contains cores 0, 2, 4, 6 and processor 1 contains cores 1, 3, 5, "
         "7,\n"
      << "  the sorted list is 0, 2, 4, 6, 1, 3, 5, 7 and the argument 0-3 "
         "will select\n"
      << "  the 4 cores of processor 0 (with physical id 0, 2, 4, 6). This "
         "option\n"
      << "  allows to ignore the details of the core numbering scheme used by "
         "the kernel.\n"
      << "--use-pty\n"
      << "  use a pseudo-terminal to collect the solver output. Currently "
         "only\n"
      << "  available when lines are timestamped. Some I/O libraries "
         "(including\n"
      << "  the C library) automatically flushes the output after each line "
         "when\n"
      << "  the standard output is a terminal. There's no automatic flush "
         "when\n"
      << "  the standard output is a pipe or a plain file. See setlinebuf() "
         "for\n"
      ...
      << "  on exit, delete IPC queues that the user owns and to which the "
         "solver\n"
      << "  was the last process to read/write [may fail to delete some "
         "queues]\n"
      << "--cleanup-all-ipc-queues\n"
      << "  on exit, delete all IPC queues that the user created [will also "
         "delete\n"
...
void watchdog(int watchdogDelay) {
  ...
  sleep(watchdogDelay);
  cout << "watchdog killed runsolver after " << watchdogDelay << " seconds."
       << endl;
  kill(getpid(), SIGABRT);
}
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    while ((optc = getopt_long(argc, argv, "+o:i:w:v:C:W:R:V:M:S:O:d:h", longopts,
      ...
      case 'O':
        int activate, max;
        if (sscanf(optarg, "%d,%d", &activate, &max) != 2 || activate >= max) {
