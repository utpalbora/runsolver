...
private:
  ...
  float limitWallClockTime; // Wall clock time (in seconds)  given to solver
                            // before we stop it
  ...
  ProcessTree *procTree,
      *lastProcTree;          // to gather data about a process and its children
  ...
  bool cleanupSolverOwnIPCQueues; // delete IPC queues that the solver may have
                                  // created?
  bool cleanupAllIPCQueues; // delete all IPC queues that are owned by the user
                            // on exit
...
public:
   ...
   */
  void setTimeStamper(TimeStamper &t) { timeStamper = &t; }

  /**
   * indicate if prctl(PR_SET_CHILD_SUBREAPER,...) was used
   */
  void setChildReaper(bool cr) { hasChildReaper = cr; }
   ...
   */
  void setCleanupAllIPCQueues(bool cleanup) { cleanupAllIPCQueues = cleanup; }

  void setCPULimit(int sec) { limitCPUTime = sec; }

  void setWallClockLimit(int sec) { limitWallClockTime = sec; }
   ...
   */
  void setVSizeLimit(long limit) { limitVSize = limit; }
   ...
   * soft limit= limit (calls stopSolver())
   */
  void setMemoryLimit(long limit) { limitMemory = limit; }

  /**
   * save the main statistics to a given file (text format)
   */
  void setVarOutputFile(char *filename) { varOutputFilename = filename; }

  /**
   * save the main statistics to a given file (binary format)
   */
  void setBinVarOutputFile(char *filename) { binVarOutputFilename = filename; }
...
#warning print the last history?
    ...
    cout << "CPU usage (%): "
         << ((wcTime != 0) ? 100 * solverCPUTime / wcTime : 100) << endl;
    ...
    strncpy(execSummary.runsolverVersion, VERSION,
            sizeof(execSummary.runsolverVersion));
    ...
    if (binVarOutputFilename) {
      ...
      if (fd < 0)
        ...
        void *p = mmap(NULL, sizeof(ExecutionSummary), PROT_READ | PROT_WRITE,
                       MAP_SHARED, fd, 0);
  ...
  void operator()() {
    ...
    if (!pt.solverEnded()) {
      cout << "WARNING: unexpectedly, some solver processes are still present "
              "and will be killed:"
           << endl;
...
protected:
  ...
  bool readProcessData(bool updateChildrenList = true) {
...
#ifdef debug
    cout << "Reading process data (updateChildrenList=" << updateChildrenList
         << ")" << endl;
#endif
    ...
    if (timeStamper)
      timeStamper->setCPUtimeFromAnotherThread(currentCPUTime +
                                               completedCPUTime);
